...
const LANG = {
  en: {
    ...
    totalSalaryTip: "The husband's and wife's gross annual salaries added together, before any deduction.",
    wifeShareLabel: "Wife's share of income",
    wifeShareTip: 'The share of the household salary the wife earns. The husband earns whatever is left, so 30% here means he earns 70%.',
    deductionLabel: 'Combined Annual Deductions (Pengurang)',
    deductionTip: 'Allowable deductions such as biaya jabatan or iuran pensiun. They come off gross income before PTKP is applied, and they are split between the spouses in proportion to what each earns.',
  ...
  id: {
    ...
    totalSalaryTip: 'Gaji kotor tahunan suami dan istri dijumlahkan, sebelum dikurangi apa pun.',
    wifeShareLabel: 'Porsi penghasilan istri',
    wifeShareTip: 'Porsi gaji rumah tangga yang diperoleh istri. Suami memperoleh sisanya, jadi 30% di sini berarti suami memperoleh 70%.',
    deductionLabel: 'Total Potongan Tahunan (Pengurang)',
    deductionTip: 'Pengurang yang diperbolehkan seperti biaya jabatan atau iuran pensiun. Dikurangkan dari penghasilan kotor sebelum PTKP, lalu dibagi antara suami dan istri sesuai porsi penghasilan masing-masing.',
...
/* ── Charts ── */
/* Each entry carries the mark its series is drawn with — a solid line for the
   two tax curves, the dashed grey rule for the unbroken difference — so the
   key reads as the chart looks. */
function buildLegend(id, series){
  const el=$(id);el.innerHTML='';
  series.forEach(s=>{
    const item=document.createElement('div');item.className='legend-item';
    SharedLegend.attach(item, s.spec || {color:cssVar(s.colorVar), width:2.5}, s.label);
    el.appendChild(item);
  });
}
...
function downloadChartPng(canvasId, filename, chartTitle, legendId, shouldDownload = true) {
  ...
  // Visible entries with the mark each one draws, so the exported key and the
  // page's key are the same key.
